...
export const Route = createFileRoute("/login")({
  head: () => ({ meta: [{ title: "Iniciar sesion · SkilltoMoney" }, { name: "description", content: "Accede a tu cuenta de SkilltoMoney" }] }),
  component: LoginPage,
});
...
function LoginPage() {
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      navigate({ to: response.data.user.account_type === "mype" ? "/dashboard/client" : "/dashboard/freelancer" });
    } catch (err: unknown) {
      const payload = err as { message?: string; errors?: Record<string, string[]> };
      setError(payload?.errors?.email?.[0] ?? payload?.message ?? "No se pudo iniciar sesion.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AuthLayout
      ...
      subtitle="Inicia sesion para seguir creciendo con tus skills."
      footer={<>Aun no tienes cuenta? <Link to="/register" className="font-semibold text-primary hover:underline">Registrate gratis</Link></>}
    >
